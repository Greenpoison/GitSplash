...
#[tauri::command]
pub async fn unskip_worktree(state: State<'_, AppState>, repo_id: String, paths: Vec<String>) -> AppResult<()> {
    let repo = repo_path(&state, &repo_id).await?;
    git::commit::unskip_worktree(&repo, &paths).await.map_err(AppError::Git)
}

#[tauri::command]
pub async fn list_skip_worktree_files(state: State<'_, AppState>, repo_id: String) -> AppResult<Vec<String>> {
    let repo = repo_path(&state, &repo_id).await?;
    git::commit::list_skip_worktree_files(&repo).await.map_err(AppError::Git)
}
