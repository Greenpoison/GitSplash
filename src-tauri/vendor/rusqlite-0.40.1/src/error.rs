...
pub unsafe fn error_with_offset(db: *mut ffi::sqlite3, code: c_int, sql: &str) -> Error {
    // GitSplash patch: `cfg_select!` is still gated behind an unstable std
    // feature this crate never declares `#![feature(cfg_select)]` for, so it
    // fails on any current stable rustc — rewritten as the equivalent
    // `#[cfg(...)]`-gated `let` pair.
    #[cfg(feature = "modern_sqlite")] // SQLite >= 3.38.0
    let result = if db.is_null() {
    ...
    let result = error_from_handle(db, code);
    result
}
