  ...
  return (
    ...
    >
      ...
      <span className="hidden shrink-0 gap-0.5 group-hover:flex">
        {staged ? (
          <Button size="icon" variant="ghost" className="size-5" onClick={(e) => { e.stopPropagation(); onUnstage?.(); }}>
            <ArrowLeft className="size-3" />
          </Button>
        ) : (
          <>
            <Button size="icon" variant="ghost" className="size-5" onClick={(e) => { e.stopPropagation(); onDiscard?.(); }}>
              <Trash2 className="size-3" />
            </Button>
            <Button size="icon" variant="ghost" className="size-5" onClick={(e) => { e.stopPropagation(); onStage?.(); }}>
            ...
            <div className="flex flex-col gap-2">
              {diff.hunks.map((hunk, i) => (
                <DiffHunkView
                  ...
                  onDiscard={() => discardHunk(hunk.raw)}
                />
              ))}
            </div>
          )}
